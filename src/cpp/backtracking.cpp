    ...
    if(K==0 || j == grid_x.size()){
        ...
        for(int i=0; i < grid_y.size(); i++){
            if (j==0){
                ...
                backtracking_2(K-1, k_0, N, grid_x, grid_y, x, y, best, current, best_error, current_error, j+1);
                if(current.size() > 0){
                    current.pop_back();
                }
            ...
            }else if(current_error<best_error && (grid_x.size()-j+1)>=k_0-current.size()){
                
                if(grid_x[j+1]!=(*max_element(grid_x.begin(), grid_x.end()))){
                    backtracking_2(K, k_0, N, grid_x, grid_y, x, y, best, current, best_error, current_error, j+1);
                    }
...
tuple<vector<tuple<float, float>>, float> backtracking(int K, int m, int n, int N, vector<float> x, vector<float> y){
    ...
    backtracking_2(K,k_0, N, grid_x, grid_y, x, y, best, current, best_error, current_error, 0);
